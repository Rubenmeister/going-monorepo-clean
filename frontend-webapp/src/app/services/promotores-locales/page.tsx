...
function parseJwt(token: string): PromoterInfo | null {
  try {
    const base64 = token.split('.')[1].replace(/-/g, '+').replace(/_/g, '/');
    const p = JSON.parse(atob(base64));
    return {
      id: p.sub || p.userId || '',
      firstName: p.firstName || p.name || 'Promotor',
      lastName: p.lastName,
      email: p.email,
...
const SIDEBAR_NAV = [
  { icon: '🏠', label: 'Inicio',       href: '/services/promotores-locales', active: true },
  { icon: '🏺', label: 'Mis tours',    href: '/services/promotores-locales/tours' },
  { icon: '📅', label: 'Reservas',     href: '/services/promotores-locales/reservas' },
  { icon: '💳', label: 'Ganancias',    href: '/services/promotores-locales/ganancias' },
  { icon: '⭐', label: 'Reseñas',      href: '/services/promotores-locales/resenas' },
  { icon: '📄', label: 'Documentos',   href: '/services/promotores-locales/documentos' },
...
const DOCUMENTS = [
  ...
  { icon: '🗺️', label: 'Licencia de Guía de Turismo (MINTUR)',   note: 'Requerido' },
  { icon: '🏢', label: 'Patente municipal',                       note: 'Requerido' },
  { icon: '🔥', label: 'Permiso de funcionamiento (Bomberos)',    note: 'Cuando aplique' },
];

export default function PromotoresLocalesPanel() {
  const [user, setUser] = useState<PromoterInfo | null>(null);
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="w-8 h-8 border-4 border-t-transparent rounded-full animate-spin" style={{ borderColor: '#0891b2' }} />
      </div>
    );
  }
  ...
  const KPIS = [
    { label: 'Ingresos este mes',   value: stats.totalEarningsMonth != null ? `$${stats.totalEarningsMonth}` : '—', icon: '💰', color: '#0891b2', href: '/services/promotores-locales/ganancias' },
    { label: 'Tours activos',       value: stats.activeTours        != null ? String(stats.activeTours)       : '—', icon: '🗺️', color: '#0033A0', href: '/services/promotores-locales/tours' },
    { label: 'Calificación',        value: stats.avgRating          != null ? `${stats.avgRating}★`           : '—', icon: '⭐', color: '#f59e0b', href: '/services/promotores-locales/resenas' },
    { label: 'Reservas este mes',   value: stats.reservationsMonth  != null ? String(stats.reservationsMonth)  : '—', icon: '📅', color: '#16a34a', href: '/services/promotores-locales/reservas' },
  ];
      ...
      <div className="flex flex-1 max-w-screen-xl mx-auto w-full">
          ...
          <div className="p-5 border-b border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-xs px-2 py-0.5 rounded-full font-semibold text-white" style={{ backgroundColor: '#0891b2' }}>Promotor Local</span>
              </div>
            </div>
          </div>
          <nav className="flex-1 px-3 py-2 overflow-y-auto">
            {SIDEBAR_NAV.map(item => (
              <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
                ...
                style={item.active ? { backgroundColor: '#0891b2' } : {}}>
        ...
        <main className="flex-1 min-w-0 px-4 lg:px-8 py-6 space-y-6">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm mt-0.5">Gestiona tus tours y experiencias locales</p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { icon: '➕', label: 'Crear tour',      href: '/services/promotores-locales/tours/nuevo' },
              { icon: '📅', label: 'Ver reservas',    href: '/services/promotores-locales/reservas' },
              { icon: '💳', label: 'Mis ganancias',   href: '/services/promotores-locales/ganancias' },
              { icon: '⭐', label: 'Reseñas',         href: '/services/promotores-locales/resenas' },
            ].map(a => (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/services/promotores-locales/reservas" className="text-xs font-bold" style={{ color: '#0891b2' }}>Ver todas →</Link>
            </div>
            {reservations.length > 0 ? (
                ...
                {reservations.map(r => (
                  <li key={r.id} className="flex items-center justify-between py-3 border-b border-gray-50 last:border-0">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{r.tourName ?? '—'} · {r.date ?? '—'}</p>
                    </div>
                    <span className="text-xs px-2 py-1 rounded-full font-semibold bg-cyan-50 text-cyan-700">{r.status ?? 'Confirmada'}</span>
                  </li>
                ))}
              </ul>
            ) : (
              <div className="text-center py-8">
                ...
                <p className="text-gray-400 text-sm">Aún no tienes tours publicados.</p>
                <Link href="/services/promotores-locales/tours/nuevo"
                  className="inline-block mt-3 text-sm font-bold underline" style={{ color: '#0891b2' }}>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/services/promotores-locales/documentos" className="text-xs font-bold" style={{ color: '#0891b2' }}>Gestionar →</Link>
            </div>
