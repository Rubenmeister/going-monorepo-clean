 ...
 *   - cleanup_handoffs     → cierra handoffs estancados sin operador (idempotent)
 *
...
export class CommandController {
  ...
  constructor(
    ...
    private readonly conversations: MongoConversationRepository,
  ) {}
  ...
  async handleCommand(@Body() body: CommandBody): Promise<CommandResponse> {
    ...
    try {
      switch (body.action) {
        ...
        case 'cleanup_handoffs':
          return await this.cleanupHandoffs(body.decisionId, body.payload || {});
        default:
  ...
  /**
   * Limpia handoffs estancados sin operadorId. Cuando MyCortex detecta
   * tickets fantasma de épocas previas (ej: handoffs de cuando aún no
   * existía la guardia operativa), propone cerrarlos en bulk.
   *
   * Conservador: solo toca handoffs que NO tienen operatorId asignado —
   * si hay alguien atendiéndolo, no interferimos. Default 7 días, ajustable
   * via payload.olderThanDays.
   *
   * Idempotente: re-llamar con el mismo decisionId no cambia outcome
   * (el segundo run encuentra los conversations ya en status='resolved'
   * y deja resolvedCount=0).
   */
  private async cleanupHandoffs(
    decisionId: string,
    payload: Record<string, unknown>,
  ): Promise<CommandResponse> {
    const olderThanDays = typeof payload.olderThanDays === 'number'
      ? payload.olderThanDays
      : 7;
    const olderThanMs = olderThanDays * 24 * 60 * 60 * 1000;

    const resolvedCount = await this.conversations.resolveStaleHandoffs(olderThanMs);
    this.logger.log(
      `[cleanup_handoffs] decision=${decisionId.slice(0, 8)} olderThanDays=${olderThanDays} resolved=${resolvedCount}`,
    );
    return {
      ok: true,
      data: { resolvedCount, olderThanDays },
    };
  }
