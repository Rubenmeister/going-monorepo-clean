...
## Anexo — versión curada por Rubén (canónica del asistente)

> Este anexo recoge el texto oficial que el asistente comparte con el público.
> Complementa lo anterior. Donde haya diferencia de detalle (contacto de
> privacidad, plazos de conservación), gana esta versión curada.
...
- Datos de método de pago (procesados por pasarelas certificadas — Going App no
...
- Conversaciones con soporte (chat de texto y, cuando autorices, grabaciones de
  voz para mejorar el servicio).
- Datos técnicos del dispositivo (modelo, sistema operativo, versión de la app —
  para soporte técnico).

**Datos que NO recolectamos**

- Contactos de tu teléfono (Going App no accede a tu agenda).
- Galería de fotos completa (solo la foto que tú subas).
- Información de tarjetas que no usas en Going App.
- Datos biométricos (no usamos Face ID/Touch ID más allá del autenticador del
  propio sistema operativo).

### Para qué usamos tus datos

- **Operar el servicio:** matching con conductores, cálculo de tarifa,
  procesamiento de pago, tracking.
- **Seguridad:** verificación de identidad, fraude, SOS.
- **Soporte:** atender consultas y resolver incidentes.
- **Facturación electrónica** según exigencias del SRI.
- **Mejorar el servicio:** análisis agregado (no individual) de patrones de uso.
- **Comunicaciones:** notificaciones del viaje (siempre), promociones (solo si
  las aceptas).
...
- **Conductoras/conductores:** lo mínimo necesario para hacer el viaje (nombre +
  dirección de pickup). Se borra de su lado al terminar el viaje.
- **Pasarelas de pago:** Datafast, DeUna, procesadora de tarjetas — solo lo que
  necesitan para cobrar.
...
- **Proveedores de infraestructura:** Google Cloud, MongoDB Atlas, Mapbox,
  Twilio, OpenAI — todos bajo contrato de confidencialidad y procesamiento de
  datos.

### Cuánto tiempo guardamos tus datos

- **Datos de cuenta:** mientras la cuenta esté activa + 5 años después del cierre
  (por exigencias contables y fiscales).
...
Como titular de los datos, puedes en cualquier momento: acceder, rectificar,
eliminar (derecho al olvido, con excepciones legales como datos fiscales en
curso), oponerte a cierto tratamiento, portabilidad y revocar consentimientos.

**Cómo ejercer estos derechos:** escribe a legal@goingec.com con copia de tu
cédula. Tenemos hasta 15 días hábiles para responder.

### Cambios a esta política

Going puede actualizar esta política. Cambios materiales se notifican con al menos
15 días de anticipación por email.

### Reclamos

Si consideras que Going App violó tu privacidad, puedes escribirnos a
legal@goingec.com o acudir a la Superintendencia de Protección de Datos
Personales del Ecuador cuando esté en operación según la LOPDP.
