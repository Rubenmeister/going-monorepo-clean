 ...
 *
 * El flujo es:
 *   1. Usuario hace login (email/password) → POST a /api/auth/login
 ...
 *   4. Logout: POST a /api/auth/logout (limpia cookie httpOnly) +
 *      clearSession() limpia localStorage + redirect.
 *   5. Refresh: POST a /api/auth/refresh (renueva token + cookie).
 *
 * NOTA IMPORTANTE: la cookie de sesión la maneja exclusivamente el server
 * (proxies en /api/auth/*). Este módulo no toca document.cookie.
 */
 ...
 * exclusivamente desde el server vía /api/auth/logout — esta
 ...
 *
 * Llama al proxy server-side /api/auth/login que internamente:
 ...
 * Llama al proxy /api/auth/mfa-verify-login con { mfaToken, code }.
 ...
 *
 * Llama al proxy server-side /api/auth/logout que limpia la cookie
 ...
 *
 * Llama al proxy server-side /api/auth/refresh que reenvía al
