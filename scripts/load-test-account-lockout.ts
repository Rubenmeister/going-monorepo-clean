 ...
 *
 * Execution: npx ts-node scripts/load-test-account-lockout.ts
 * Expected Duration: 60 seconds
 *
 * Scenarios:
 * 1. Single user - 10 failed attempts (verify lockout after 5)
 * 2. 100 concurrent users - 5 attempts each (verify exponential backoff)
 * 3. Spike test - 1000 concurrent login attempts
 * 4. Recovery test - verify auto-unlock after configured duration
 */
...
class MockAccountLockoutService {
  ...
  private lockoutMultiplier = 1.5;
  private maxLockoutMinutes = 480; // 8 hours
  private inMemoryLockout = new Map<string, any>(); // Fallback if Redis unavailable
...
async function testConcurrentUsers() {
  ...
  const promises: Promise<LockoutResult>[] = [];
